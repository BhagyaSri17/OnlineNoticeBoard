...
<body>
    ...
    $uploads_dir = 'C:\xampp2\htdocs\MiniProject\student\image';
    move_uploaded_file($tname, $uploads_dir.'/'.$pname);

        $query = mysqli_query($connect, "update student set
        name='$name',email='$email', phno='$phno',
        password='$password',dept='$dept',year='$year',section='$section' ,image='$pname'  where id='$id'");
    ...
    <div class="content1">
        ...
        if (isset($_POST['update'])) {
            $update = $_POST['update'];
            $query1 = mysqli_query($connect, "select * from student where id='$update'");
        ...
        echo "<div class='round'>";
          ...
          echo "</div>";
                ...
                echo "<input class='input' type='text' name='dept' value='{$row1['dept']}' />";


                echo "<div class='input-container'>
    <div class='input-block'>
    <label for='year'>Year</label>
    <input class='input-inline' type='text' name='year' value='{$row1['year']}' />
    </div>
    <div class='input-block'>
    <label for='section'>Section</label>
    <input class='input-inline' type='text' name='section' value='{$row1['section']}' />
    </div>
</div>";
