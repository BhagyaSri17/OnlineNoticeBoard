...
$q=mysqli_query($connect,"select * from hod");
...
<?php include 'admin_index.php';?>
    ...
    <?php
if(!$rr){
    echo '<span style="font-size:25px;">No HOD Data  available.!</span>';?>
  <div >
    <button style="float:none;" id="add_hod">Add HOD</button></div>
...
else
{
    ...
    <div>
    <h2><center>HOD Details</center></h2>
    <div ><button id="add_hod">Add HOD</button></div>
    <button id="showCheckBox">Delete HOD</button>
    ...
    <?php include 'hodsearch.html';?>
    <div class="tablewrapper">
            <table class="table table_border">
                <thead>
                <tr class="heading">
                    <th><button  id="del_hod" class="recordCheckbox" name="delete_hod" style="display:none;">Delete</button></th>
                    <th>SNo.</th>
                    <th> Id</th>
                    <th> Name</th>
                    <th> Email</th>
    ...
    <?php
        if(isset($_POST['search'])){
            $search=$_POST['search'];
            $res=mysqli_query($connect,"select * from hod where id= '$search' or name='$search' or email= '$search' or phno='$search' or dept='$search' or post='$search'");
...
style="fill:#FFFFFF;">
    ...
    <span class="form-heading"><center>HOD Registration Form</center></span>
</div>
    <form action="register_hod.php" method="post">
...
<script>
    var btn=document.getElementById("add_hod");
...
<script>
    ...
    document.getElementById("back").addEventListener('click',function(){
        window.location.href="hodmanagement.php";
    })
</script>
<script>
    document.getElementById("del_hod").addEventListener('click',function (){
                    ...
                    if(confirm("Do you want to delete?"))
          {
                    ...
                    window.location.href = "delete_hod.php?ids=" +encodeURIComponent(uid);
                } 
            }
                else {
